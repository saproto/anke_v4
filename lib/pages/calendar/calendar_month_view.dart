...
class _CalendarPageState extends State<CalendarMonthViewPage> {
  ...
  void initState() {
    ...
    _selectedEvents = _getActivtiesForCurrentDay();
  ...
  List<dynamic> _getActivtiesForCurrentDay() {
    List<Activity> currentActivities = _activityListService.current;
    List<dynamic> activitiesForCurrentDay = [];
    for (var activity in currentActivities) {
      if (stripTimeFromDateTime(activity.startDate) ==
          stripTimeFromDateTime(DateTime.now())) {
        activitiesForCurrentDay.add(activity);
      }
    }
                        ...
                        selectedDayBuilder: (context, date, events) =>
                            Container(
                                ...
                                child: Text(date.day.toString(),
                                    style: usedTextStyle(Colors.white)))),
                  ),
...
DaysOfWeekStyle _daysOfWeekStyle() {
  return new DaysOfWeekStyle(weekendStyle: usedTextStyle(Color(0xff6c757d)));
}
...
TextStyle usedTextStyle(Color color, {bool header = false}) {
  ...
  return new TextStyle(color: color, fontFamily: 'Poppins', fontSize: size);
}
